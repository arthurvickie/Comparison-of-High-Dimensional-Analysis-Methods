...
##############################################################################
cv.SCAD_ADMM_re <- function(X, Y, N, beta0, K=5, rho=1, a=3.7, err=0.5e-3, tune="cv", unpen){
...
## K-folded cross validation, K=5 by default
##############################################################################
cv.SCAD_ADMM_unre <- function(X, Y, N, beta0, K=5, rho=1, a=3.7, err=0.5e-3, tune="cv", unpen){
...
simu.ex2 <- function(L=200, n=100, p=50, h=0, beta=2, sig=1, rho=0){
  ...
  for (l in 1:L){
    ...
    beta.unre <- cv.SCAD_ADMM_unre(X=Model$X, Y=Model$Y, N=N, beta0=rep(0, dim(Model$X)[2]), err=1e-4, tune="BIC", unpen = c(49))
    indice.unre <- beta.unre!=0
    
    # estimate the constrained estimator
    beta.re <- cv.SCAD_ADMM_re(X=Model$X, Y=Model$Y, N=N, beta0=rep(0, dim(Model$X)[2]), err=1e-4, tune="BIC", unpen = c(47))
    indice.re <- beta.re!=0
